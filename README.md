...
This repo builds and publishes a shared PyPI library used across the [Creators Dashboard](https://github.com/agile-crafts-people/impact) system.
...
## run demo dev server - captures command line, serves API at localhost:9094
## Note: ENABLE_LOGIN=true is automatically set by the dev script
pipenv run dev

## run E2E tests (assumes running API at localhost:9094)
pipenv run e2e
...
# Server will be available at http://localhost:9094
```

### API Explorer

Visit **http://localhost:9094/docs/explorer.html** for an interactive API explorer with:
...
TOKEN=$(curl -s -X POST http://localhost:9094/dev-login \
  -H "Content-Type: application/json" \
  -d '{"subject": "user-123", "roles": ["admin"]}' | jq -r '.access_token')

# Get configuration
curl http://localhost:9094/api/config \
